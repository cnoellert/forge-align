...
conda run -n "$ENV_NAME" pip install -e "$SCRIPT_DIR" --quiet
...
if [[ -w "$CONFIG_DIR" ]] || [[ ! -d "$CONFIG_DIR" ]]; then
    ...
    if [[ -w "$CONFIG_DIR" ]] || mkdir -p "$CONFIG_DIR" 2>/dev/null; then
        ...
        echo "  NOTE: Cannot write to $CONFIG_DIR — run with sudo or:"
        echo "    sudo mkdir -p $CONFIG_DIR && sudo chown \$USER $CONFIG_DIR"
        echo "  The hook will fall back to: ~/miniconda3/envs/forge/bin/python"
    fi
else
    echo "  NOTE: Cannot write to $CONFIG_DIR — run with sudo or:"
    echo "    sudo mkdir -p $CONFIG_DIR && sudo chown \$USER $CONFIG_DIR"
...
# ── Step 4: Deploy hook to Flame project ──────────────────────────
echo ""
...
if [[ -n "$PROJECT_PATH" ]]; then
    ...
    # Copy only the hook script (forge_cv runs from conda, not from Flame)
    cp "$SCRIPT_DIR/scripts/forge_cv_align.py" "$DEST/scripts/"
    ...
    echo "  mkdir -p /path/to/project/setups/python/$HOOK_NAME/scripts"
    echo "  cp $SCRIPT_DIR/scripts/forge_cv_align.py /path/to/project/setups/python/$HOOK_NAME/scripts/"
fi
